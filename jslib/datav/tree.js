...
define(function (require, exports, module) {
   ...
   //-------Array2JSON-----------begin
    Tree.prototype._arrayToJson = function (array) {
   ...
   //-------Array2JSON-----------end
    ...
    Tree.prototype.setSource = function (source) {
        ...
        //alert(this._arrayToJson(source));
        //alert(source instanceof Array);
        //判断数据源类型(数组 或 json)
    ...
    //------------------------begin
    Tree.prototype._createTreemap = function () {
        var conf = this.defaults;
    ...
    Tree.prototype.generatePaths = function () {
        ...
        nodes.forEach(function (d, i) {
			...
			d.click(function(){
				//alert(nodesData[i].name +"==" + nodesData[i].num);
				document.getElementById("oldNodeName").value = nodesData[i].name;////所要增加节点的名称
				document.getElementById("nodeId").value = nodesData[i].num;//对应的隐藏域ID
				document.getElementById("add_oldNodeId").value = nodesData[i].name;////所要增加节点的名称(添加用户)
				document.getElementById("add_nodeId").value = nodesData[i].num;//对应的隐藏域ID
			});
			//单击事件end
            ...
            d.mouseover(function () {
                ...
                var getFline = function (node, num) {
                    ...
                    if (parent) {
                        path[node.id - 1].attr({"stroke-width": 4, "stroke-opacity": num});//stroke-width：调整线条粗细
                        if ( num > 0.5) {
                            num = num - 0.1;
                        }
                        getFline(parent, num);
