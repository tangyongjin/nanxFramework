...
Ext.extend(Ext.ux.form.TimePickerField, Ext.form.Field, {
    ...
    _valueSplit: function(v){
        ...
        if(v.getHours)
         {
        var hms_str=v.getHours()+':'+v.getMinutes()+':'+v.getSeconds();
        var split = hms_str.split(':');
         }
         else
          {
             var split = v.split(':');
    ...
    getValue: function(){
        ...
        String.leftPad(v.m,2,'0')+':'+String.leftPad(v.s,2,'0');
    },
    setValue: function(value){
        ...
        var hms = this._valueSplit(value);
        this.setRawValue(hms);
        this.validate();
    }
});
...
Ext.DateTimePicker = Ext.extend(Ext.DatePicker, {
    ...
    setValue: function(value){
        if (!this.tf) {
            this.tf = new Ext.ux.form.TimePickerField({value:value});
            this.tf.ownerCt = this;
        }
        this.value = this.getDateTime(value);
    },
    getDateTime:function(value){
        console.log('value send to getDatetime is---->');
        console.log(value);
        
        if (this.tf)
        {    
            console.log('this has tf');
            if( value.format && value.length>=10 ) 
            {
            console.log('value can format');
            console.log(this.tf.getValue());
            value = Date.parseDate(value.format(this.dateFormat)+' '+this.tf.getValue(),this.format);
            console.log(value);
            }
            else
            {
            console.log('value set to date()');
             value=new Date();
            }
        }
        else
          { 
             console.log('this does not has tf');
             value=new Date();
          }
...
Ext.ux.form.DateTimeField = Ext.extend(Ext.form.DateField, {
    ...
    getValue: function(){
         if(this.beforeBlur==Ext.emptyFn)
         {
         return this.el.dom.value;
         }
         else
        {       
        var raw=this.parseDate(Ext.form.DateField.superclass.getValue.call(this)) || '';
        return  raw;
       }
    },
    onTriggerClick: function(){
        ...
        var raw2=this.getValue()||new Date();
        this.menu.picker.setValue(raw2);
