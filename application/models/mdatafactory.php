...
class MDatafactory extends CI_Model
{
    ...
    $join_str = '';
		...
		{
	    if(	strlen($ghost)>10){$ghosts_str.= $ghost . ",";}
		}
  ...
  function getTransedField($basetable,$field,$combo_fileds)
  {
			...
			if ($field == 'pid')
			{
					...
					$join='';
					return  array( 'join' =>'','transed' => $transed,'ghost'=>'' );
			}
			...
			reset($combo_fileds);
			return  array( 'join' => $join ,'transed' => $transed,'ghost'=> $ghost );
  }
  
	function getDatabyBasetable($table,$filter_field,$filter_value,$pid_order,$query_cfg)
	{
	  $transfer=true;
    ...
    $all_fields=$this->db->query("show full fields from  $table")->result_array();
	  $all_fields=array_retrieve($all_fields,array('Field','Type'));
	...
	{ 
     ...
     if($operator=='like'){$arg='%'.$arg.'%';}
    if($operator=='like_end'){ $operator='like';   $arg=$arg.'%';}
    if($operator=='like_begin'){$operator='like';$arg='%'.$arg;}
