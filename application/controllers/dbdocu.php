...
foreach ($tables as $table)
{
   ...
   function lang_js()
    {
     ...
     $e = fopen('C:\wamp\www\newoss\js\language\en\i18n.js','r');
     $e_arr=array();
     while(!feof($e))
     {
      $line=fgets($e);
     ...
     $c = fopen('C:\wamp\www\newoss\js\language\zh-cn\i18n.js','r');
     $c_arr=array();
     while(!feof($c))
     {
      $line=fgets($c);
     ...
     $sql="select  concat('copy  en\\\i18n.js   ',id,'\\\i18n.js   /Y' ) as vv from  nanx_lang where id not in ('en','zh-cn');";
    ...
    {
     $this->load->helper('file');
     $e = fopen('C:\wamp\www\newoss\application\language\en\messages_lang.php','r');
     $e_arr=array();
     while(!feof($e))
     {
      $line=fgets($e);
     ...
     $c = fopen('C:\wamp\www\newoss\application\language\zh-cn\messages_lang.php','r');
     $c_arr=array();
     while(!feof($c))
     {
      $line=fgets($c);
     ...
     $sql="select  concat('copy  en\\\messages_lang.php   ',id,'\\\messages_lang.php   /Y' ) as vv from  nanx_lang where id not in ('en','zh-cn');";
     $bat=$this->db->query($sql)->result_array();
     //debug($bat);
     foreach($bat as $cp)
      {
  ...
  print json_encode($cfg, JSON_UNESCAPED_UNICODE);
  $this->load->helper('file');
